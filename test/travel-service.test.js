...
test("travel mode starts exactly one sleep inhibitor and stops it", () => {
  ...
  const service = new TravelService({ spawnProcess:(command, args) => { calls += 1; assert.equal(command, "systemd-inhibit"); assert.deepEqual(args, ["--what=sleep", "--mode=block", "--why=Vertex Travel Mode", "sleep", "infinity"]); return child; } });
