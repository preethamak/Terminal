...
const SKIP = new Set([".git", ".cache", ".config", ".local", ".npm", ".cargo", ".rustup", "node_modules", "vendor", "dist", "build", "target"]);

class ProjectIndex {
  constructor({ root = process.env.HOME || ".", file = path.join(process.env.HOME || ".", ".vertex", "projects.json"), maxDepth = 4 } = {}) {
    this.root = root; this.file = file; this.maxDepth = maxDepth;
  }
  ...
  async refresh() {
    const found = []; const walk = async (directory, depth) => {
      ...
      await Promise.all(entries.filter((entry) => entry.isDirectory() && !SKIP.has(entry.name)).map((entry) => walk(path.join(directory, entry.name), depth + 1)));
    };
