...
function ProjectGrid({ projects, onOpen, onDiscover }) { const [scanning, setScanning] = useState(false); if (!projects.length) return <button className="empty-card project-scan" onClick={async () => { if (!onDiscover || scanning) return; setScanning(true); try { await onDiscover(); } finally { setScanning(false); } }}><span>⌕</span><div><strong>{scanning ? "Scanning your laptop…" : "Find projects on laptop"}</strong><small>Vertex securely finds Git projects in your home folder. No paths to type.</small></div><b>{scanning ? "…" : "→"}</b></button>; return <div className="project-grid">{projects.slice(0, 4).map((project, index) => <button className={`project-card c${index % 4}`} key={project.path} onClick={() => onOpen(project)}><span className="project-icon">{initials(project.name)}</span><strong>{project.name}</strong><small>{project.branch || "main"}</small><i>→</i></button>)}</div>; }
