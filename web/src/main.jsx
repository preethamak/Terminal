...
function WorkspaceGrid({ workspaces, onOpen, onNew, limit = 4 }) { if (!workspaces.length) return <button className="empty-card workspace-empty" onClick={onNew}><span>+</span><div><strong>Create your first workspace</strong><small>Make a normal folder or a Git project on your laptop.</small></div><b>→</b></button>; return <div className="workspace-grid">{workspaces.slice(0, limit).map((workspace) => <button key={workspace.path} onClick={() => onOpen(workspace)}><span>{workspace.kind === "git" ? "⌘" : "□"}</span><div><strong>{workspace.name}</strong><small>{workspace.source === "session" ? "Open in terminal now" : workspace.kind === "git" ? "Git workspace" : "Folder workspace"}</small></div><b>→</b></button>)}<button className="workspace-new" onClick={onNew}><span>+</span><strong>New workspace</strong></button></div>; }
