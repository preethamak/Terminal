...
function QrScanner({ close }) { const video = useRef(null); const [message, setMessage] = useState("Opening camera…"); useEffect(() => { let stream; let frame; let stopped = false; const start = async () => { if (!("BarcodeDetector" in window)) return setMessage("QR scanning needs a current Android Chrome or Nativine WebView. Update the app, then try again."); try { stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:{ ideal:"environment" } }, audio:false }); if (stopped) return; video.current.srcObject = stream; await video.current.play(); const detector = new BarcodeDetector({ formats:["qr_code"] }); setMessage("Point the camera at the QR on your laptop."); const scan = async () => { if (stopped) return; try { const codes = await detector.detect(video.current); const value = codes[0]?.rawValue; if (value?.includes("relayPair=")) { stopped = true; window.location.assign(value); return; } } catch { /* A frame can be unavailable while the camera starts. */ } frame = requestAnimationFrame(scan); }; scan(); } catch { setMessage("Camera permission was blocked. Allow camera access for Vertex and try again."); } }; start(); return () => { stopped = true; cancelAnimationFrame(frame); stream?.getTracks().forEach((track) => track.stop()); }; }, []); return <div className="scanner-backdrop"><section className="scanner-card"><button className="sheet-close" onClick={close}>×</button><p className="kicker">PAIR VERTEX</p><h2>Scan the laptop QR</h2><video ref={video} className="scanner-video" muted playsInline/><p>{message}</p></section></div>; }
