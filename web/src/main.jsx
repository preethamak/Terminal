...
function Welcome({ onConnect }) { const [token, setToken] = useState(""); const [scanner, setScanner] = useState(false); const [copied, setCopied] = useState(false); const copyCommand = async () => { try { await navigator.clipboard.writeText("cd ~/vertex && npm run pair"); setCopied(true); setTimeout(() => setCopied(false), 1800); } catch { setCopied(false); } }; return <main className="welcome"><div className="orb orb-one"/><div className="orb orb-two"/><div className="welcome-card"><Brand/><div className="welcome-copy"><p className="kicker">YOUR LAPTOP, IN YOUR POCKET</p><h1>Your terminal,<br/><em>wherever you are.</em></h1><p>Pair once, then open your laptop’s real terminal securely from this phone.</p></div><div className="pair-card pair-guide"><span className="pair-icon">1</span><div><strong>On your laptop, run this</strong><div className="pair-command"><code>cd ~/vertex && npm run pair</code><button onClick={copyCommand}>{copied ? "Copied" : "Copy"}</button></div><small>It restarts Vertex and prints a fresh QR code.</small></div></div><div className="pair-card pair-guide"><span className="pair-icon">2</span><div><strong>Scan that QR here</strong><small>Tap below, allow camera access, then point it at the laptop screen.</small></div></div><button className="primary-button welcome-scan" onClick={() => setScanner(true)}>Scan pairing QR <span>⌗</span></button><details className="development-token"><summary>Use a development token instead</summary><label className="token-label">Local testing token<input value={token} onChange={(event) => setToken(event.target.value)} placeholder="Paste only for direct local testing" type="password" /></label><button className="secondary-button" onClick={() => token && onConnect(token)}>Connect with token <span>→</span></button></details><p className="welcome-foot">Your code stays on your laptop. Always.</p></div>{scanner && <QrScanner close={() => setScanner(false)}/>}</main>; }
