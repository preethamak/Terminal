...
function SessionList({ sessions, onOpen, onManage }) { if (!sessions.length) return <p className="muted-copy">No open terminal sessions yet. Open Codex or a shell in tmux to continue it here.</p>; return <div className="sessions">{sessions.map((session) => <div className="session-row" key={session.name}><button onClick={() => onOpen(session)}><span className="session-terminal">›_</span><span><strong>{session.name}</strong><small>{session.program ? `${session.program} · ${shortPath(session.cwd)}` : session.attached ? "Active now" : "Ready to resume"}</small></span><b>→</b></button><button className="session-manage" aria-label={`Manage ${session.name}`} onClick={() => onManage(session)}>•••</button></div>)}</div>; }
