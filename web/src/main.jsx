...
function useVertex() {
  ...
  const [data, setData] = useState({ tasks:[], sessions:[], projects:[], activities:[], health:null });
  ...
  const refresh = useCallback(async () => {
    try {
      const [sessions, tasks, initialProjects, activity, health] = await Promise.all([request("list"), request("listTasks"), request("listProjects"), request("listActivity"), request("getHealth")]);
      const projects = initialProjects.projects?.length ? initialProjects : await request("refreshProjects");
      setData({ sessions:sessions.sessions || [], tasks:tasks.tasks || [], projects:projects.projects || [], activities:activity.activities || [], health }); setStatus("Laptop online");
  ...
  const forget = useCallback(() => { localStorage.removeItem("vertex.token"); localStorage.removeItem("vertex.relay"); setToken(null); setRelay(null); setData({ tasks:[], sessions:[], projects:[], activities:[], health:null }); }, []);
...
function ProjectsPanel({ vertex }) { const [query, setQuery] = useState(""); const projects = vertex.data.projects.filter((project) => project.name.toLowerCase().includes(query.toLowerCase())); return <><p className="kicker">WORKSPACES</p><h2>Choose a project</h2><input className="project-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects"/>{!projects.length ? <ProjectGrid projects={projects} onOpen={() => {}} onDiscover={vertex.discoverProjects}/> : <ProjectGrid projects={projects} onOpen={() => {}} onDiscover={vertex.discoverProjects}/>}<p className="muted-copy">Choose a workspace from the dashboard to start a new task in it.</p></>; }
...
function TaskComposer({ project, vertex, close }) {
  const [task, setTask] = useState({ cli:"codex", name:"", cwd:project?.path || "", prompt:"" }); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
...
function TerminalView({ vertex, session, onClose }) {
  const element = useRef(null); const socket = useRef(null); const terminal = useRef(null); const finder = useRef(null); const output = useRef({ expected:1, pending:new Map(), scheduled:false }); const touchY = useRef(null);
  const [status, setStatus] = useState("Connecting…"); const [following, setFollowing] = useState(true); const [searchOpen, setSearchOpen] = useState(false); const [query, setQuery] = useState("");
  useEffect(() => {
    ...
    (async () => {
      ...
      const addon = new FitAddon(); const searchAddon = new SearchAddon(); term.loadAddon(addon); term.loadAddon(searchAddon); term.open(element.current); terminal.current = term; finder.current = searchAddon;
      ...
      cleanup = () => { vertex.terminalListener.current = () => {}; observer.disconnect(); dataListener.dispose(); scrollListener.dispose(); element.current?.removeEventListener("touchstart", onTouchStart); element.current?.removeEventListener("touchmove", onTouchMove); element.current?.removeEventListener("touchend", onTouchEnd); socket.current?.close(); term.dispose(); finder.current = null; };
    })().catch((error) => setStatus(error.message));
