...
function useVertex() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [sessions, tasks, projects] = await Promise.all([request("list"), request("listTasks"), request("listProjects")]);
...
function Welcome({ onConnect }) { const [token, setToken] = useState(""); return <main className="welcome"><div className="orb orb-one"/><div className="orb orb-two"/><div className="welcome-card"><Brand/><div className="welcome-copy"><p className="kicker">DEVELOPMENT, UNBOUND</p><h1>Your terminal,<br/><em>wherever you are.</em></h1><p>Start work on your laptop. Continue it naturally from Vertex—without moving your code to the cloud.</p></div><div className="pair-card"><span className="pair-icon">⌘</span><div><strong>Pair your laptop</strong><small>Scan the Vertex QR code to get started.</small></div></div><label className="token-label">Development token<input value={token} onChange={(event) => setToken(event.target.value)} placeholder="Paste only for local testing" type="password" /></label><button className="primary-button" onClick={() => token && onConnect(token)}>Connect Vertex <span>→</span></button><p className="welcome-foot">Your code stays on your laptop. Always.</p></div></main>; }
